...
class BayesNet {
	...
	int count_sample(vector<int>& sample, long double weight, double timer)
	{
		if(timer < 2) return -1;
	...
	inline int weight_of_sample(int sample_index, int unkown_index,double timer)
	{
	...
	void EM(double timer,string f)
	{
		double start_time = (double) time(NULL);
		init_cpt(lmda);
		normalize_cpt();
		create_bif(f,"solved_alarm.bif");
		while((double) time(NULL) - start_time < timer)
		{
			...
			for(int s = 0 ; s < samples.size(); s++)	
			{
				...
				int ret = weight_of_sample(s,unknown_index[s], timer - ((double) time(NULL) -start_time) );
				if(ret == -1) return;
			...
			for(int s = 0; s < samples.size(); s++)
			{
				...
				{
					int ret = count_sample(samples[s],1, timer - ((double) time(NULL) - start_time));
					if(ret == -1) return;
				}
				else
				{
					...
					{
						samples[s][node] = i;
						int ret = count_sample(samples[s],sample_weight[s][i], timer - ((double) time(NULL) -start_time));
						if(ret == -1) return;
					}
					samples[s][node] = -1;
				}
...
void BayesNet::create_bif(string f1, string filename)
	{
		...
		while(getline(fin,line))	
		{
			...
			if(first == "probability")
			{
				...
				ss >> paren >> paren;
				fout << "\t"<<"table ";
				for(auto x : cpt[id[paren]]) fout << fixed << setprecision(2)<<  x << " ";
				fout << ";\n";
			}
		}
	}
